...
import { mentoringApi, communityApi, gamificationApi, chatApi } from '../api/client';
    ...
    try {
      ...
      setDashboardData({
        recentPosts: postsRes?.data && Array.isArray(postsRes.data) ? postsRes.data.slice(0, 3) : 
                     (postsRes?.data?.content && Array.isArray(postsRes.data.content) ? postsRes.data.content.slice(0, 3) : []),
        mentoringRequests: mentoringRes?.data && Array.isArray(mentoringRes.data) ? mentoringRes.data.slice(0, 5) : [],
        chatRooms: chatRes?.data && Array.isArray(chatRes.data) ? chatRes.data.slice(0, 5) : [],
              ...
              <div className="lg:col-span-2 space-y-10">
                ...
                <div className="bg-white shadow-2xl rounded-[3rem] border border-gray-100 overflow-hidden">
                  <div className="px-10 py-8 border-b border-gray-50 flex items-center justify-between bg-gradient-to-r from-white to-gray-50/30">
                    ...
                    <Link to="/chat" className="text-sm font-black text-primary-600 hover:text-primary-800 bg-primary-50 px-4 py-2 rounded-xl transition-all">
                      전체보기
                    </Link>
                  </div>
                  <ul className="divide-y divide-gray-50">
                    ...
                    ) : (dashboardData.chatRooms.length > 0 || dashboardData.mentoringRequests.length > 0) ? (
                      <>
                        {/* Show Active Chats first */}
                        {dashboardData.chatRooms.map((room) => (
                          <li key={`chat-${room.roomId}`} className="px-10 py-6 hover:bg-primary-50/30 transition-all group">
                            <Link to={`/chat/rooms/${room.roomId}`} className="flex items-center justify-between">
                              <div className="flex items-center flex-1 min-w-0">
                                <div className="h-14 w-14 rounded-2xl bg-primary-50 flex items-center justify-center border border-primary-100 group-hover:border-primary-300 group-hover:bg-white transition-all flex-shrink-0">
                                  <MessageSquare className="h-7 w-7 text-primary-600" />
                                </div>
                                <div className="ml-6 flex-1 min-w-0">
                                  <div className="flex items-center gap-2">
                                    <p className="text-lg font-black text-gray-900 group-hover:text-primary-700 transition-colors truncate">
                                      {room.studentName === user.name ? room.seniorName : room.studentName}
                                    </p>
                                    <span className="px-2 py-0.5 bg-primary-100 text-primary-700 text-[10px] font-black rounded-md uppercase">채팅 중</span>
                                  </div>
                              ...
                              <div className="ml-4 flex-shrink-0 text-right">
                                <p className="text-xs font-black text-gray-400 flex items-center justify-end">
                        ...
                        {/* Then show Mentoring Requests that aren't yet in chat (or just show all) */}
                        {dashboardData.mentoringRequests
                          .filter(req => !dashboardData.chatRooms.some(room => room.seniorId === req.mentorId || room.studentId === req.menteeId))
                          .map((req) => (
                          <li key={`req-${req.id}`} className="px-10 py-6 hover:bg-primary-50/30 transition-all group">
                      ...
                      dashboardData.recentPosts.map((post) => (
                        <Link key={post.id} to={`/community/${post.id}`} className="block px-8 py-6 hover:bg-primary-50/30 transition-all group">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <p className="text-base font-black text-gray-900 truncate group-hover:text-primary-700">{post.title}</p>
                          </div>
                          <div className="flex items-center text-[10px] font-black uppercase tracking-widest text-gray-400">
                            <span>{post.anonymous ? '익명' : post.authorName}</span>
